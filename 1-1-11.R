...
# *****************************************************************************************************************
# *****************************************************************************************************************
# META's recent 8-K in HTML
# https://www.sec.gov/Archives/edgar/data/1326801/000132680122000015/fb-12312021xexhibit991.htm

# get data by passing in url & headers
pg <- GET(url = paste0("https://www.sec.gov/Archives/edgar/data/1326801/000132680122000015/fb-12312021xexhibit991.htm"),
          config = httr::add_headers(`User-Agent` = PASS$usrAgent,
                                     `Accept-Encoding` = 'gzip, deflate'))

# raw data
pg_raw <- content(pg, as="text", encoding="UTF-8", type = "text/html") %>% read_html()

pg_raw %>% html_nodes("body") %>% html_nodes("tbody")

pg_raw %>% html_nodes("body") %>% html_nodes("table") %>% .[[1]] %>% html_table %>% View()
