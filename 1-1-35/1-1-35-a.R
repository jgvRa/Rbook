...
# helper function to get shares outstanding via barChart
getSharesOut = function(ticker){
  url = paste0("https://www.barchart.com/stocks/quotes/",gsub("\\-",".",ticker),"/overview")
  so <- read_html(url)
  so <- so %>% html_nodes(xpath="//*[@id='main-content-column']/div/div[4]/div[2]/div/div[1]/ul/li[2]/span[2]") %>% html_text()
  so <- gsub("\\,","",gsub("[[:space:]]","",so))
  so<- as.numeric(so)*1000
  so
}
# ****************************************************************************************************************
#                                        Read in CIK Codes : 
...
getLatestBL = function(ticker)
{
  ...
  df <- unique(df) %>% as.data.frame() %>% unique
...
getRestBL = function(ticker)
{
  ...
  df = lapply(as.list(1:length(pg)), function(ii){
    ...
    # get rid of special characters
    for(ii in 2:ncol(df)){
      df[,ii] <- gsub("N/A",NA,gsub("\\$","",gsub("\\,","",df[,ii])))
    }
    ...
    # remove ALL empty rows
    df = df[!(rowSums(is.na(df)) == (ncol(df)-1)),]#df = df[!(rowSums(is.na(df)) == 5),]
    # remove duplicated rows
    df <- unique(df) %>% as.data.frame() %>% unique
    # make sure it is data.frame
    df <- data.frame(df[,2:ncol(df)],row.names = df$Description)
    # return data.frame
    df
  })
  if(length(pg) > 1){
...
getBLRatios = function(BL,ticker)
{
  ...
  tmp = Cl(to.quarterly(getSymbols(gsub("\\.","-",ticker), auto.assign = FALSE, from="2000-01-01")))
  if(nrow(tmp)<100){
    tmp = Cl(to.quarterly(getSymbolsDB(gsub("\\.","-",ticker))))
  }
  ...
  for(ii in 1:ncol(BL))
  {
    ...
    if(inherits(prc,'try-error') | is.null(prc))
    {
...
getLatestCF = function(ticker)
{
  ...
  dup = which(df$Description == "Operating Cash Flow")
  if(length(dup) > 1)
  {
    df$Description[last(dup)] <- "Operating CF"
  }else{
    df$Description[dup[1]] <- "Operating CF"  
  }
  # make sure it is data.frame
  df <- data.frame(df[,2:6],row.names = df$Description)
  # return data.frame
...
getRestCF = function(ticker)
{
  ...
  df = lapply(as.list(1:length(pg)), function(ii){
    ...
    # remove ALL empty rows
    df = df[!(rowSums(is.na(df)) == (ncol(df)-1)),]#;df =  df[which(df$Description != ""),]
    # change duplicate row name
    dup = which(df$Description == "Operating Cash Flow")
    if(length(dup) > 1)
    {
    ...
    dup = which(df$Description == "Free Cash Flow")
    if(length(dup) > 1)
    {
      df$Description[last(dup)] <- "FCF"
    }
