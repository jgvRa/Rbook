...
saveRDS(all_etfs,"all_etfs.rds")
all_etfs <- readRDS("all_etfs.rds")
all_etfs$`Shares Held` <- as.numeric(all_etfs$`Shares Held`) %>% suppressWarnings() %>% suppressMessages()
saveDATE <- RQuantLib::advance(calendar = "UnitedStates/NYSE",dates = Sys.Date(),n = -1,timeUnit = 0)
all_etfs$Date <- saveDATE
saveRDS(all_etfs,paste0("/Volumes/6TB/constituents/spdrs/",format(as.Date(saveDATE),"%Y%m%d"),".rds"))
...
invTickers = as.character(invProd$Ticker)

invALL = lapply(as.list(invTickers), function(x){
  tmp <- try(getInvescoETF(ticker = x),silent = TRUE)
  if(!inherits(tmp,'try-error'))
    tmp
})
# row bind results
invETFS <- rbindlist(invALL,use.names = TRUE,fill=TRUE)
invETFS <- as.data.frame(invETFS)

invETFS$Shares.Par.Value <- as.numeric(invETFS$Shares.Par.Value)
invETFS$MarketValue <- as.numeric(invETFS$MarketValue)
invETFS$Weight <- as.numeric(invETFS$Weight)
invETFS$Date <- as.character(as.Date(invETFS$Date,"%m/%d/%Y"))
saveDATE <-unique(invETFS$Date)[1]
# saveRDS
saveRDS(invETFS,paste0("/Volumes/6TB/constituents/invesco/",format(as.Date(saveDATE),"%Y%m%d"),".rds"))
...
# format spreadsheet and save as CSV
iShares = read.csv("product-screener.csv",header = TRUE,sep = ",")


getiSharesETF = function(iShares,ticker){
  Sys.sleep(5)
  ...
  tmp <- subset(iShares, iShares$Ticker == ticker)
  URL = paste0(tmp$Name,"/1467271812596.ajax?fileType=csv&fileName=",x,"_holdings&dataType=fund")
  ETF <- read.csv(URL,skip = 9,header = TRUE)
  # complete cases only
  ETF <- ETF[complete.cases(ETF$Weight....),]
  # add ETF
  ETF$etf <- ticker
  # format columns
  ETF$Market.Value <- as.numeric(gsub(",","",ETF$Market.Value)) %>% suppressWarnings() %>% suppressMessages()
  ETF$Weight.... <- as.numeric(ETF$Weight....)/100 %>% suppressWarnings() %>% suppressMessages()
  ETF$Notional.Value <- as.numeric(gsub(",","",ETF$Notional.Value)) %>% suppressWarnings() %>% suppressMessages()
  ETF$Shares <- as.numeric(gsub(",","",ETF$Shares)) %>% suppressWarnings() %>% suppressMessages()
  ETF$Price <- as.numeric(ETF$Price) %>% suppressWarnings() %>% suppressMessages()
...
# test function
ETF = unique(iShares$Ticker)
ETF <- ETF[str_length(ETF)>0]

ALL = lapply(as.list(ETF), function(x){
  #cat("\n",x)
  tmp <- try(getiSharesETF(iShares = iShares, ticker = x),silent = TRUE)
  if(!inherits(tmp,'try-error'))
    tmp
})
# row bind results
ETFS <- rbindlist(ALL,use.names = TRUE,fill=TRUE)
ETFS <- as.data.frame(ETFS)
ETFS$Date <- saveDATE
# saveRDS
saveRDS(ETFS,paste0("/Volumes/6TB/constituents/iShares/",format(as.Date(saveDATE),"%Y%m%d"),".rds"))

# summarise
ISHARES = ETFS %>% group_by(Ticker) %>% summarise(totalShares = sum(Shares,na.rm = TRUE),
                                                  etfCount = n(),
                                                  avgWt = mean(Weight....,na.rm = TRUE),
                                                  sumWt = sum(Weight....,na.rm = TRUE),
                                                  nValue = sum(Notional.Value,na.rm = TRUE))


ISHARES <- ISHARES[order(ISHARES$nValue,decreasing = TRUE),]
ISHARES$sValue <- scales::label_number(accuracy=0.01, scale_cut=scales::cut_short_scale())(ISHARES$nValue)
