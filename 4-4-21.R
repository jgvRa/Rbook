...
breakOut = function(df, lookBackDays, closeAtHigh=FALSE)
{
  ...
  df$VoAvg <- rollmean(x=Vo(df),k = lookBackDays, align = "right")
  
  # start at row # (lookBackDays) to see how many times is todays Cl price
  # higher than previous lookBack Days
  # df$nDaysHigher = NA
  # 
  # tmp <- as.data.frame(coredata(df$MAX[1:lookBackDays]))
  # tmp$CLOSE <- rep(coredata(Cl(df)[lookBackDays]), nrow(tmp))
  # df$nDaysHigher[lookBackDays]   <- nrow(tmp[tmp$CLOSE >= tmp$MAX,])
  # 
  # STOP = nrow(df) - lookBackDays
  # 
  # for(ii in 1:STOP)
  # {
  #   tmp <- as.data.frame(coredata(df$MAX[(ii+1):(lookBackDays+ii)]))
  #   tmp$CLOSE <- rep(coredata(Cl(df)[lookBackDays+ii]), nrow(tmp))
  #   df$nDaysHigher[lookBackDays+ii]   <- nrow(tmp[tmp$CLOSE >= tmp$MAX,])
  # }
  # 
  ...
  if(closeAtHigh == TRUE)
  {
    breakOutDates =  df[which(df$nDaysHigher == lookBackDays & Vo(df) > df$VoAvg),]
  }
...
getStats = function(boPerformance,ii){
############## 1st Week Stats
...
# View results
breakOuts = breakOut(df=df,lookBackDays=149,closeAtHigh=FALSE)
boPerformance = breakOutPerformance(df=df,breakOuts=breakOuts)
# ****************************************************************************************************
# ****************************************************************************************************
# calculate geometric means of a range of 'lookBackDays' 
...
dbDollarVol <- dbDollarVol[dbDollarVol$Symbol != c("DJI","DJT","SP600","BRK.A"),]

# top 50 stocks
tickers <- dbDollarVol$Symbol[1:500]
...
oneWK_Sharpe <- subset(ALL, ALL$sharpe1 > 0.80 & ALL$g2l_1WK > 0.70)
oneWK_Sharpe <- oneWK_Sharpe[,c("N","oneWK","g2l_1WK","sharpe1","Symbol")]
